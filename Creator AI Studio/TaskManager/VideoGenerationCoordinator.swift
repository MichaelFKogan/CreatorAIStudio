...
class VideoGenerationCoordinator: ObservableObject {
    ...
    func canCancelTask(notificationId: UUID) -> Bool {
        ...
        print("🔍 [VideoGenerationCoordinator] canCancelTask called for notificationId: \(notificationId)")
        print("🔍 [VideoGenerationCoordinator] Currently tracking \(generationTasks.count) tasks:")
        ...
        guard let taskInfo = generationTasks.values.first(where: { $0.notificationId == notificationId }) else {
            print("🔍 [VideoGenerationCoordinator] canCancelTask: task NOT FOUND for notificationId: \(notificationId)")
            return false
        }
        ...
        print("🔍 [VideoGenerationCoordinator] canCancelTask: task FOUND, returning true for notificationId: \(notificationId)")
        return true
    }
