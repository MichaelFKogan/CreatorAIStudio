...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (provider === "unknown") {
      ...
      if ((payload.request_id || payload.requestId || payload.gateway_request_id) && 
          (payload.status === "OK" || payload.status === "ERROR")) {
        detectedProvider = "falai";
        console.log(`[Webhook] Auto-detected provider as falai from payload structure`);
      }
      // Check for Runware structure: has taskUUID
      else if (payload.taskUUID || payload.task_uuid || payload.data?.[0]?.taskUUID) {
        detectedProvider = "runware";
        console.log(`[Webhook] Auto-detected provider as runware from payload structure`);
      }
      // Check for WaveSpeed structure: has id
      else if (payload.id && (payload.status === "completed" || payload.status === "failed")) {
        detectedProvider = "wavespeed";
        console.log(`[Webhook] Auto-detected provider as wavespeed from payload structure`);
      }
    }
    
    // Use detected provider if we auto-detected one
    const finalProvider = detectedProvider !== "unknown" ? detectedProvider : provider;
    ...
    if (finalProvider === "runware") {
      ...
      console.log(`[Webhook] Fal.ai - Full payload:`, JSON.stringify(payload, null, 2));
      console.log(`[Webhook] Fal.ai - Payload keys:`, Object.keys(payload).join(", "));
      ...
      console.log(`[Webhook] Fal.ai - Extracted taskId: ${taskId || "NOT FOUND"}`);
      if (!taskId) {
        console.error(`[Webhook] Fal.ai - Could not find taskId in payload. Available keys:`, Object.keys(payload));
      }
      console.log(`[Webhook] Fal.ai - Status: ${payload.status}`);
      console.log(`[Webhook] Fal.ai - Has payload: ${!!payload.payload}`);
      console.log(`[Webhook] Fal.ai - Payload keys: ${payload.payload ? Object.keys(payload.payload).join(", ") : "none"}`);
      ...
      if (payload.status === "OK") {
        ...
        if (payload.payload?.video?.url) {
          ...
          console.log(`[Webhook] Fal.ai - Payload structure:`, JSON.stringify(payload.payload, null, 2));
          // Try alternative structure - maybe video is at top level?
          if (payload.video?.url) {
            status = "completed";
            resultUrl = payload.video.url;
            errorMessage = null;
            console.log(`[Webhook] Fal.ai - Video URL found at top level: ${resultUrl}`);
          } else {
            status = "processing";
            resultUrl = null;
            errorMessage = null;
            console.log(`[Webhook] Fal.ai - Status OK but no video URL found, marking as processing`);
          }
        }
      } else if (payload.status === "ERROR") {
        status = "failed";
        errorMessage = payload.error || payload.payload?.detail || "Video generation failed";
        resultUrl = null;
        console.log(`[Webhook] Fal.ai - Error: ${errorMessage}`);
      } else if (payload.status === "IN_PROGRESS" || payload.status === "IN_QUEUE") {
        status = "processing";
        resultUrl = null;
        errorMessage = null;
        ...
        resultUrl = null;
        errorMessage = null;
        console.log(`[Webhook] Fal.ai - Unknown status: ${payload.status}, defaulting to processing`);
      }
